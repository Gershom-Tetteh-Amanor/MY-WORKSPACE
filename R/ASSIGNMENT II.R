Sys.time()
Collaboratores <- data.frame("NAMES" = c("GERSHOM TETTEH AMANOR","EVORDA KOKOU JOHN    "),
                             "ID_NUMBER"=c("10967696","10971696"));Collaboratores
# Question 1
...
# Question 6
x <- c(12,32,43,9,7,5);x
y <- sample(1:100,size=6);y
z <- round(runif(6,0,50),2);z
my.data <- data.frame(x,y,z,
           row.names=LETTERS[1:6])

x.order <- sort(x,decreasing = F)

# Using the first column to order the entire data frame
my.data.ordered <- rbind.data.frame(
  my.data[my.data$x==x.order[1],],
  my.data[my.data$x==x.order[2],],
  my.data[my.data$x==x.order[3],],
  my.data[my.data$x==x.order[4],],
  my.data[my.data$x==x.order[5],],
  my.data[my.data$x==x.order[6],]
);my.data.ordered
...
States <- data.frame("abb"=state.abb,"are"=state.area,
                     "div"=state.division,"reg"=state.region,
...
State.and.x77.simplified1[,7] <- level.of.illiteracy

colnames(State.and.x77.simplified1) <- c(
  colnames(State.and.x77.simplified1[,-7]),
